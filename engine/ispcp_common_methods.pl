...
=head2 generateRandomChars(length, exclude)

Generates a random string with specified length generated by the pseudo number generator.
...
sub generateRandomChars {
	...
	push_el(\@main::el, 'generateRandomChars()', 'Starting...');

	my ($len, $exclude) = @_;

	if (!defined $len || $len eq '') {
		push_el(
			\@main::el, 'generateRandomChars()',
			"[ERROR] Undefined input data, len: |$len| !"
		);
	...
	my ($i, $rdata, $rc, $asciiValue) = (0, undef, undef, undef);
	...
	while ($i <= $len) {
        ...
        # Test all chars from the $exclude list
		foreach (@excludeList) {
		    if (ord($_) == ord($rc)) {
                # Set ASCII value to a disallowed value
		        $asciiValue = 0;
		    }
		}
		# Excludes all chars below Space (incl.) and above ~, the escape char (\) and the '
		next if ($asciiValue <= 32 || $asciiValue >= 126 || $asciiValue == 92 || $asciiValue == 39);
	...
	push_el(\@main::el, 'generateRandomChars()', 'Ending...');
