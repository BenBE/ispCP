...
// Hack for ispCP: ftpserver always 'localhost', check the username for the host and drop the "htdocs" folder in the path
        elseif (($net2ftp_globals['ftpserver'] == 'localhost') && (strpos($_REQUEST['username'], '@') !== false)) {
                // take ftp-username to get the domainname
                list(, $userdomain) = explode('@', $_REQUEST['username']);
                // remove htdocs from the directory if available (otherwise -> error!!!)
                $specialdirectories[1] = 'htdocs';
                for ($i=1; $i<=sizeof($specialdirectories); $i++) {
                        $pos = strpos($directory, $specialdirectories[$i]);
                        if ($pos !== false) {
                ...
                // error if not in htdocs...
                if ($pos == false) {
                        $list_links[1] = 'You are not in the "htdocs" Webfolder - you cannot access this folder from the web...';
                } else {
                // create the URL
                for ($i=1; $i<=sizeof($list_files); $i++) {
                        $URL = 'http://www.' . $userdomain . $directory . '/' . $list_files[$i][$encoding];
                        if ($htmltags == 'no') { $list_links[$i] = $URL; }
                        else                   { $list_links[$i] = "<a href=\"" . $URL . "\" target=\"_blank\" title=\"" .
                                                __("Execute %1\$s in a new window", $list_files[$i][$encoding]) . "\">" . $list_files[$i][$encoding] . "</a>"; }
                } // end for
                }
        }
// -------------------------------------------------------------------------
