 ...
 * @version		SVN: $Id: hosting_plan_update.php 1740 2009-05-06 22:12:01Z haeber $
...
function add_new_order(&$tpl, &$sql, $order_id, $user_id) {
	...
	$message = sprintf(tr("You have an update order for the account %s\n\nPlease login into your ispCP control panel at %s for more details", true),
...
if (UserIO::GET_Int('delete_id') > 0) {
	del_order($tpl, $sql, UserIO::GET_Int('delete_id'), $_SESSION['user_id']);
}

if (UserIO::GET_Int('order_id') > 0) {
	add_new_order($tpl, $sql, UserIO::GET_Int('order_id'), $_SESSION['user_id']);
}
