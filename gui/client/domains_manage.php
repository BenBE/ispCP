 ...
 * @version		SVN: $Id: domains_manage.php 1740 2009-05-06 22:12:01Z haeber $
...
function gen_user_dns_list(&$tpl, &$sql, $user_id) {
	...
	if ($rs->RecordCount() == 0) {
		...
		while (!$rs->EOF) {
			...
			$tpl->assign(
				array(
					'DNS_DOMAIN'				=> UserIO::HTML($domain_name),
					'DNS_NAME'					=> UserIO::HTML($sbd_name),
					'DNS_CLASS'					=> UserIO::HTML($rs->fields['domain_class']),
					'DNS_TYPE'					=> UserIO::HTML($rs->fields['domain_type']),
					'DNS_DATA'					=> UserIO::HTML($sbd_data),
//					'DNS_ACTION_SCRIPT_EDIT'	=> $sub_action,
					...
					'DNS_ACTION_EDIT'			=> tr("%s record", $rs->fields['domain_type'])
				)
			);
...
";
	...
	if (($rs->RecordCount() + $rs2->RecordCount()) == 0) {
		...
		while (!$rs->EOF) {
			...
			$tpl->assign(
				array(
					'SUB_NAME'			=> UserIO::HTML($sbd_name),
					'SUB_ALIAS_NAME'	=> UserIO::HTML($rs->fields['domain_name']),
					'SUB_MOUNT'			=> UserIO::HTML($rs->fields['subdomain_mount']),
		...
		while (!$rs2->EOF) {
			...
			$tpl->assign(
				array(
					'SUB_NAME'			=> UserIO::HTML($sbd_name),
					'SUB_ALIAS_NAME'	=> UserIO::HTML($rs2->fields['alias_name']),
					'SUB_MOUNT'			=> UserIO::HTML($rs2->fields['subdomain_alias_mount']),
		...
		while (!$rs->EOF) {
			...
			$tpl->assign(
				array(
					...
					'ALS_MOUNT'			=> UserIO::HTML($rs->fields['alias_mount']),
					'ALS_STATUS'		=> translate_dmn_status($rs->fields['alias_status']),
					'ALS_FORWARD'		=> UserIO::HTML($als_forward),
