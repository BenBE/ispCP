 ...
 * @version		SVN: $Id: protected_user_manage.php 1740 2009-05-06 22:12:01Z haeber $
...
function gen_pusres(&$tpl, &$sql, &$dmn_id) {
	...
	if ($rs->RecordCount() == 0) {
		...
		while (!$rs->EOF) {
			...
			$tpl->assign(
				array(
					'UNAME'					=> UserIO::HTML($rs->fields['uname']),
...
//			$members = $rs->fields['members'];
			...
			$tpl->assign(
				array(
					'GNAME'					=> UserIO::HTML($rs->fields['ugroup']),
			...
			if ($rs->fields['members'] == '') {
				...
				for ($i = 0, $cnt_members = count($members); $i < $cnt_members; $i++) {
					...
					if ($cnt_members == 1 || $cnt_members == $i + 1) {
						$tpl->assign('MEMBER', UserIO::HTML($rs_members->fields['uname']));
					} else {
						$tpl->assign('MEMBER', UserIO::HTML($rs_members->fields['uname'] . ", "));
					}
