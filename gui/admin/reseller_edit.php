...
function update_reseller() {
	...
	if($rdata['domain_software_allowed'] == "no") {
		...
		if (!is_dir($cfg->GUI_SOFTWARE_DIR."/".$rdata['edit_id']."/")) {
			@mkdir($cfg->GUI_SOFTWARE_DIR."/".$rdata['edit_id'],0755,true);
			#@chown($cfg->GUI_SOFTWARE_DIR."/".$rdata['edit_id'], "vu2000");
			#@chgrp($cfg->GUI_SOFTWARE_DIR."/".$rdata['edit_id'], "www-data");
		}
	}
	...
	exec_query(
		...
		array(
			...
			$rdata['domain_software_allowed'], $rdata['domain_softwaredepot_allowed'],
			$rdata['edit_id']
		)
	);
...
$tpl->assign(
	array(
		...
		'SOFTWARE_YES' => ($rdata['software_allowed'] == 'yes') ? 'selected="selected"' : '',
		'SOFTWARE_NO' => ($rdata['software_allowed'] != 'yes') ? 'selected="selected"' : '',
		'SOFTWAREDEPOT_YES'	=> ($rdata['softwaredepot_allowed'] == 'yes') ? 'selected="selected"' : '',
		'SOFTWAREDEPOT_NO' => ($rdata['softwaredepot_allowed'] != 'yes') ? 'selected="selected"' : '',
