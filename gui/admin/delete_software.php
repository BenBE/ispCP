...
$tpl = new pTemplate();
$tpl->define_dynamic('page', Config::get('ADMIN_TEMPLATE_PATH') . '/delete_software.tpl');
...
if (isset($_GET['id']) || isset($_POST['id'])) {
	...
	if($rs->fields['software_depot'] == "yes") {
		$del_path = Config::get('GUI_SOFTWARE_DEPOT_DIR')."/".$rs->fields['software_archive']."-".$rs->fields['software_id'].".tar.gz";
		@unlink($del_path);
		...
		if(isset($_POST['id']) && is_numeric($_POST['id']) && $_POST['uaction'] === 'send_delmessage') {
			if (!empty($_POST['id']) && !empty($_POST['delete_msg_text'])) {
				...
				$del_path = Config::get('GUI_SOFTWARE_DIR')."/".$rs->fields['reseller_id']."/".$rs->fields['software_archive']."-".$rs->fields['software_id'].".tar.gz";
				@unlink($del_path);
				$delete="DELETE FROM `web_software` WHERE `software_id` = ?";
				$res = exec_query($sql, $delete, array($rs->fields['software_id']));
				set_page_message(tr('Software was deletet.'));
