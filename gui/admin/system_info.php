 ...
 * @version		SVN: $Id: system_info.php 1740 2009-05-06 22:12:01Z haeber $
...
function gen_mount_point(&$tpl) {
	...
	while (list($number, $row) = each($mount_points)) {
		...
		$tpl->assign(
			array(
				'MOUNT'		=> UserIO::HTML($row['mount']),
				'TYPE'		=> UserIO::HTML($row['fstype']),
				'PARTITION'	=> UserIO::HTML($row['disk']),
				'PERCENT'	=> UserIO::HTML($row['percent']),
...
$tpl->assign(
	array(
		...
		'CPU_MODEL'				=> UserIO::HTML($cpu['model']),
		'CPU_COUNT'				=> UserIO::HTML($cpu['cpus']),
		'CPU_MHZ'				=> UserIO::HTML($cpu['cpuspeed']),
		'CPU_CACHE'				=> UserIO::HTML($cpu['cache']),
		'CPU_BOGOMIPS'			=> UserIO::HTML($cpu['bogomips']),
		...
		'TR_LOAD'				=> tr('Load'),
		'LOAD'					=> UserIO::HTML($load['avg'][0] . ' ' . $load['avg'][1] . ' ' . $load['avg'][2]),
		'RAM'					=> tr('RAM'),
