...
function restore_form(&$tpl, &$sql) {
	$tpl->assign(
		array(
			...
			'TR_SOFTWARE_YES' => ($_POST['software_allowed'] == '_yes_') ? Config::getInstance()->get('HTML_CHECKED') : '',
			'TR_SOFTWARE_NO' => ($_POST['software_allowed'] == '_no_') ? Config::getInstance()->get('HTML_CHECKED') : ''
		)
	);
...
function gen_load_ehp_page(&$tpl, &$sql, $hpid, $admin_id) {
	...
	$tpl->assign(
		array(
			...
			'TR_SOFTWARE_YES' => ($hp_allowsoftware == '_yes_') ? Config::getInstance()->get('HTML_CHECKED') : '',
			'TR_SOFTWARE_NO' => ($hp_allowsoftware == '_no_' || !$hp_allowsoftware) ? Config::getInstance()->get('HTML_CHECKED') : ''
		)
	);
