 ...
 * @version		SVN: $Id: alias.php 1740 2009-05-06 22:12:01Z haeber $
...
function generate_als_list(&$tpl, $reseller_id, &$als_err) {
	...
	$start_index = $current_psi = UserIO::GET_Int('psi');
	...
	if (UserIO::POST_String('uaction', true, true) !== false) {
		...
		if (isset($_SESSION['search_for']) && !UserIO::GET_isset('psi')) {
	...
	$tpl->assign(
		array(
			'PSI'				=> $current_psi,
			'SEARCH_FOR'		=> UserIO::HTML($search_for),
			'TR_SEARCH'			=> tr('Search'),
