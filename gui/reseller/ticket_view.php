 ...
 * @version		SVN: $Id: ticket_view.php 1744 2009-05-07 03:21:47Z haeber $
...
function change_ticket_status_view($sql, $ticket_id) {
	...
	if (UserIO::POST_isset('uaction')
		&& UserIO::POST_GetString('uaction') != "open"
		&& UserIO::POST_GetString('uaction') != "close") {
		if ($ticket_level != 2) {
			// if ticket to user
			$ticket_status = 2;
		} else {
...
if (UserIO::GET_Int('ticket_id') > 0) {
	if (UserIO::GET_Int('screenwidth') > 0) {
		$screenwidth = UserIO::GET_Int('screenwidth');
	} else if (UserIO::POST_Int('screenwidth') > 0) {
		$screenwidth = UserIO::POST_Int('screenwidth');
	} else {
		$screenwidth = 1024;
	}
	...
	change_ticket_status_view($sql, UserIO::GET_Int('ticket_id'));

	send_user_message($sql, $_SESSION['user_id'], $reseller_id, UserIO::GET_Int('ticket_id'), $screenwidth);

	gen_tickets_list($tpl, $sql, UserIO::GET_Int('ticket_id'), $screenwidth);
} else {
	set_page_message(tr('Ticket not found!'));
