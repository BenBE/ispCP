...
function smarty_function_control_input_text($params, $smarty, $template)
{
	...
	$result = '<input type="text" class="textinput" value="';
	$result .= htmlentities($params['value'], ENT_COMPAT, 'UTF-8')
			. '" name="'.$params['name'].'" '.$params['addition_html']
			. ' />';

	//	$smarty->tpl_vars['control'] = $params;
	//	$result = $smarty->fetch($smarty->root_template_path . 'controls/control_input_text.tpl');
